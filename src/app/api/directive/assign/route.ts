...
{"agents":[{"id":"agent_id","task":"brief task description"}]}`;
        ...
        if (jsonMatch) {
          ...
          if (parsed.agents && Array.isArray(parsed.agents) && parsed.agents.length > 0) {
            ...
            if (validAgents.length >= 2) {
              if (!validAgents.find((a: { id: string }) => a.id === 'lead')) {
                validAgents.push({ id: 'lead', task: 'Synthesis & briefing' });
              }
              if (validAgents.length < 3 && !validAgents.find((a: { id: string }) => a.id === 'risk')) {
                validAgents.push({ id: 'risk', task: 'Risk assessment' });
              }
              if (validAgents.length < 3 && !validAgents.find((a: { id: string }) => a.id === 'research')) {
                validAgents.push({ id: 'research', task: 'Research' });
              }
              return NextResponse.json({ agents: validAgents, source: 'openrouter' });
            }
          }
        }
      }
    ...
    const validIds = new Set(activeAgents.map(a => a.id));
    return NextResponse.json({ agents: fallbackAssign(title, description || '').filter(a => validIds.has(a.id)), source: 'fallback' });
  } catch {
